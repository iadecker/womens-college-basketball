...
The original source data had many issues pertaining to the position column. There were issues of positions being listed as either a single letter or spelled out. Other players had double-listed positions. There were also various manager fields and fields with the player's height. All in all, the source data needed a lot of cleaning.

To standardize the positions, we wrote the data frame out to a new csv so that we could manually edit the positions column in a google spreadsheet. After standardizing the positions, we wrote a new csv and loaded that into our Rmd before joining the original and cleaned dataframes.
